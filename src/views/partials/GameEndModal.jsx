...
import gameWinAudio from "../../assets/game-win-music.wav";
...
const GameEndModal = ({ gameEndResults }) => {
  ...
  useEffect(() => {
    fetch(
      ...
      }/game/check-if-top-ten/${imageId}/${playerId}`,
      {
        method: "GET",
      }
    )
  ...
  return (
    <div className={styles.gameEndModalBackground}>
      ...
      <div className={styles.gameEndModalCont}>
        ...
        <p className={styles.resultsPara}>
          You took:{" "}
          {formatDistanceStrict(
            gameEndResults[0].start_time,
            gameEndResults[0].end_time,
            { includeSeconds: true, addSuffix: false }
          )}
        </p>
        {!isPlayerTopTen.madeTopTen ? null : (
          <div className={styles.topTenCont}>
            <p>You made it to top 10!</p>
            <p>Your Rank is {isPlayerTopTen.rank}</p>
            {isPlayerTopTen.rank == 1 ? (
              <>
                <audio
        ...
        <Link to="/" className={styles.tryNewGameLink}>
          Try a new game
        </Link>
      </div>
    </div>
